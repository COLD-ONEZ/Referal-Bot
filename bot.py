...
async def main():
    ...
    # IMPORTANT: pass timeout as a plain integer, NOT aiohttp.ClientTimeout.
    # aiogram's polling loop does:  int(bot.session.timeout + polling_timeout)
    # That arithmetic breaks when session.timeout is a ClientTimeout object.
    from aiogram.client.session.aiohttp import AiohttpSession
    session = AiohttpSession(timeout=60)   # seconds — plain int required
...
async def _run_webhook(bot: Bot, dp: Dispatcher):
    ...
    Telegram POSTs each update to /webhook; no persistent outbound connection needed.
    """
