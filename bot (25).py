...
async def _retry_telegram_call(coro_fn, max_retries: int = 7, base_delay: float = 5.0):
    ...
    coro_fn must be a zero-arg callable that returns a coroutine each time.
    """
...
async def on_startup(bot: Bot):
    ...
    # Cache bot username with retry — a brief network glitch no longer kills startup
...
async def main():
    ...
    # Larger timeouts to handle Koyeb → Telegram latency spikes.
    # TCPConnector with DNS cache avoids repeated DNS lookups on every request.
    ...
    try:
        ...
        # Always close the session cleanly to avoid "Unclosed client session" warnings
