...
from database.referral_events_db import record_join_event, mark_point_awarded, has_ever_joined
...
async def _award_retroactive_points(bot, user_id: int, referrer_id: int):
    """
    FIX: Called after referred_by is saved to DB.
    If the referee has already joined any required channels (join event already
    fired before /start was processed), award points retroactively so the
    referrer is not cheated out of their credit.
    """
    channels = await channels_db.get_all_channels()
    if not channels:
        return

    required_ids = {ch["channel_id"] for ch in channels}
    total_channels = len(required_ids)

    for ch in channels:
        chat_id = ch["channel_id"]

        # Skip if already tracked (points already awarded via normal flow)
        if await has_ever_joined(user_id, chat_id):
            continue

        # Check if user is actually in this channel right now
        try:
        ...
        # User is in channel but join event was missed — award now
        ...
        logger.info(
            f"Retroactive: awarded {point} points to {referrer_id} "
            f"(referee {user_id} was already in channel {chat_id})"
        )

    # Check if referee is now in ALL required channels
    db_user = await users_db.get_user(user_id)
    user_channels = set(db_user.get("channels_joined", [])) if db_user else set()
    if required_ids.issubset(user_channels):
        await users_db.add_joined_referral(referrer_id, user_id)
        await users_db.update_invites(referrer_id, 1)
        logger.info(
            f"Retroactive: referee {user_id} in ALL channels — "
    ...
    if referral_code:
        ...
        if referrer and referrer["user_id"] != user.id:
            ...
            if db_user and not db_user.get("referred_by"):
                saved = await users_db.set_referred_by(
                    user_id=user.id,
                ...
                if saved:
                    referral_saved = True
                    # FIX: Award retroactive points if referee already joined
                    # channels before /start was processed (race condition where
                    # chat_member join event fired before referred_by was saved).
                    await _award_retroactive_points(
                        message.bot, user.id, referrer["user_id"]
                    )
