...
async def handle_admin_add_channel(message: Message):
    """Process channel addition from admin."""
    if not is_admin(message.from_user.id):
        return
    ...
    if message.forward_from_chat:
        ...
        if text.lstrip("-").isdigit():
            ...
            await message.answer("❌ Please send a valid channel ID (e.g. <code>-1001234567890</code>)", parse_mode="HTML")
            return
...
async def cmd_broadcast(message: Message):
    """Start broadcast: ask admin to send the message to broadcast."""
    if not is_admin(message.from_user.id):
        return

    if message.reply_to_message:
        # Broadcast immediately from replied message
...
@router.message(F.text | F.photo | F.video | F.document | F.animation)
async def handle_broadcast_input(message: Message):
    """Capture broadcast message from admin."""
    if not is_admin(message.from_user.id):
        return
    state = await get_state(message.from_user.id)
...
async def _do_broadcast(trigger_message: Message, content_message: Message):
    ...
    for i, uid in enumerate(user_ids):
        ...
        # Batch delay to avoid hitting Telegram rate limits
...
async def cmd_leaderboard(message: Message):
    """Show top 3 users leaderboard (admin gets full data, users get truncated)."""
    # Admins get full leaderboard, everyone else can too (it's a reward feature)
    top_users = await users_db.get_leaderboard(limit=3)
    ...
    for rank, user in enumerate(top_users):
        if is_admin(message.from_user.id):
            ...
            # Public version: hide phone and UPI
