...
def _bot_username(bot) -> str:
    """Return cached username, falling back to config if cache not yet set."""
    if BOT_USERNAME_CACHE:
        return BOT_USERNAME_CACHE
    # Fallback: import from config (always available, no API call)
    try:
...
async def _handle_after_join_verified(message_or_callback, user, referral_code_from_link: str = None):
    ...
    Uses cached bot username — NO get_me() API call here.
    """
    name = escape_html(user.full_name)
    db_user = await users_db.get_user(user.id)
    username = _bot_username(message_or_callback.bot)
...
async def cmd_start(message: Message, command: CommandObject):
    ...
    # Persist referral code in FSM so check_join callback can retrieve it
    if referral_code:
...
async def callback_check_join(callback: CallbackQuery):
    ...
    await callback.answer()  # Dismiss loading spinner immediately
    ...
    if not_joined:
        # Rebuild keyboard with fresh invite links and notify user
